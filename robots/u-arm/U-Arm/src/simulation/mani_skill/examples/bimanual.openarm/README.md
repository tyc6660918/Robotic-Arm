...
本仓库在 Windows 11 + WSL2 Ubuntu 22.04 中运行 ROS 2 Humble，将两套 8 通道
leader 舵机转换为 OpenArm v1.0 标准控制器命令。默认配合 ROS 2 fake hardware 和
RViz 使用，不需要真实 OpenArm 从臂。

安装、启动、关节映射和故障排查请先阅读：

**[servo_teleop/README.md](servo_teleop/README.md)**

克隆时需要同时获取官方子模块：

```powershell
git clone --recurse-submodules https://github.com/YZZ919/openarm.git D:\openarm
```
