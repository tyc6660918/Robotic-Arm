...
关键设计（`Windows离线仿真说明.md` 第 49-58 行 + 代码核对）：
...
**windows_sim 的边界（`Windows离线仿真说明.md` 第 69-73 行，代码一致）：**
...
| # | 位置 | 问题 |
|---|---|---|
| B1 | `robot-viewer/robot_viewer.py:145` | "查看器"连接时发 `#CMDMODE 2` + `!START`，**使能全部 6 个电机**；`stop()` 只 `close()` 串口，**不发 `!DISABLE`**，退出后电机仍使能。`Windows离线仿真说明.md:72-73` 明确警告过这一点 |
...
```
⚠️ `命令行控制工具说明.md` 已过期（描述 macOS + `simple_cli.py` 下划线文件名）。
...
**改法**：`robot_viewer.py:145` 只发 `#GETJPOS` 轮询，删掉 `#CMDMODE 2` 和 `!START`；`stop()` 补 `!DISABLE`。参考 `Windows离线仿真说明.md:72-73` 已写明的约束。
...
- [ ] `windows_sim` 的只读数字影子适配器 —— **必须独立实现，只允许 `#GETJPOS`**，不要复用现有 viewer 连接逻辑（`Windows离线仿真说明.md:72-73`）
