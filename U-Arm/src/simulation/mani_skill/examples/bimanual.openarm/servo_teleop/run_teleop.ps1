...
$TeleopScript = '/mnt/d/openarm/servo_teleop/mujoco_teleop.py'
...
if ($NoReleaseTorque) {
    Write-Host 'Skipping leader torque release; sampling zero for two seconds.'
} else {
    Write-Host 'Releasing torque on all 16 leader servos, then sampling zero for two seconds.'
}
Write-Host 'Starting OpenArm MuJoCo teleoperation...'
